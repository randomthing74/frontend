...
document.getElementById("btn-download-grafik-fuzzy")?.addEventListener("click", async () => {
  try {
    ...
    // Output sebagai singleton (garis vertikal)
    ...
    // ========== RULE VIEWER & AGREGASI ==========
    const ruleY = 350, ruleH = 300, ruleW = 900, ruleX = 60;
    ctx.strokeStyle = "#000"; ctx.lineWidth = 1;
    ctx.strokeRect(ruleX, ruleY, ruleW, ruleH);

    // Row heights
    const rowH = ruleH / 5; // 5 baris: R1, R2, R3, Agregasi, Centroid
    const colors = ["#e74c3c", "#f39c12", "#27ae60"];

    // Helper untuk plot α-cut dan output singleton di rule viewer
    const plotRuleRow = (rowIdx, alphaVal, outPercent, color, ruleName) => {
      ...
      // Plot input MF kecil (kiri)
      const miniW = ruleW * 0.3;
      // (sederhana: hanya garis horizontal untuk menunjukkan α)
    ...
    // Gambar area agregasi (tiga singleton dengan tinggi α)
...
setTimeout(() => {
  switchToMonitoring();
  // Load monitoring.js secara dinamis
  import('/monitoring.js')
    .then(module => {
      // Jika monitoring.js mengekspor fungsi init, panggil di sini
      if (module.initMonitoring) {
        module.initMonitoring();
      }
      console.log('✅ monitoring.js loaded');
    })
    .catch(err => console.error('❌ Gagal load monitoring.js:', err));
}, 2000);
    }
  } catch (err) {
...
function startRealtimePolling() {
  ...
  }, 300); // ⬅️ 300 ms untuk update lebih real‑time
}
...
function drawECGLoop(timestamp) {
  ...
  // ⬇️ Ubah dari 33 ms (30 fps) menjadi 16 ms (60 fps)
